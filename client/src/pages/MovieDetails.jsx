...
return show ? ( 
        ...
        <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
            ...
            <div className="relative flex flex-col gap-3">
                ...
                <div className="flex items-center flex-wrap gap-4 mt-4">
                        ...
                        <a href="#dateSelect" className="px-10 py-3 text-sm bg-[#F84565] hover:bg-[#F84565]/80 transition rounded-md font-medium cursor-pointer active:scale-95">Buy Tickets</a>
  ...
  {show.movie.casts.slice(0, 12).map((cast, index) => (
    <div key={index} className="flex flex-col items-center">
      <img src={cast.profile_path} alt={cast.name} className="h-20 object-cover rounded-full md:h-20 aspect-square"/>
      <p className="text-xs mt-3 font-medium">{cast.name}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
                {dummyShowsData.filter(show => show._id !== id).slice(0, 4).map((movie,index) => (
                        <MovieCard key={index} movie={movie} />
                ))}
        </div>
        <div className="flex justify-center mt-8">
                <button onClick={() => {navigate('/movies'); scrollTo(0, 0);}} className="bg-[#F84565] hover:bg-[#F84565]/80 transition-all rounded-md font-medium text-sm px-10 py-3 cursor-pointer">Show More</button>
        </div>
    </div>
) : (
    <Loading/>
